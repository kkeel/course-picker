...
(function () {
  function attachPrintHelper() {
    ...
    if (!tip || !printBtn || !btnGo) return; // not on this page / not loaded yet
    ...
    function doPrint() {
      closeTip();
      setTimeout(() => {
        // Use your existing Alpine print flow (keeps your page-number logic)
        try {
          const root = document.querySelector("[x-data]");
          const x = root && root.__x;
          const data = x && x.$data;
          if (data && typeof data.printView === "function") return data.printView();
        } catch (e) {}
        window.print();
      }, 150);
    }

    // Capture-phase intercept so Alpine @click doesn't fire first
    ...
    btnCancel && btnCancel.addEventListener("click", closeTip);
  ...
  // IMPORTANT: app.js loads before the modal in your HTML, so wait for DOM
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", attachPrintHelper);
  } else {
    attachPrintHelper();
  }
})();
